"""Reproduce training with CV model selection and an untouched final test set."""
...
def run(output):
    ...
    estimators = {
        ...
        'Random Forest': RandomForestRegressor(n_estimators=140, max_depth=16,
            min_samples_split=4, min_samples_leaf=2, random_state=42, n_jobs=-1),
        'Gradient Boosting': GradientBoostingRegressor(n_estimators=140,
            learning_rate=.05, max_depth=3, random_state=42),
    }

    # Select the model using only the training partition. The final test set is
    ...
    # One final evaluation after model selection.
    prediction = best.predict(X_test)
    final_metrics = score(y_test, prediction)
    print({'Final Test Model': best_name, **final_metrics}, flush=True)
    ...
    metadata = dict(
        ...
        note='Model selection uses training-only cross-validation. The held-out test set is evaluated once after selection.'
    )
    ...
    print('PASS: provenance, train-only CV selection, final test evaluation and model reload', flush=True)
    return metadata
