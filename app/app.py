...
with pred_tab:
    ...
    with c2:
        ...
        pickup_lat=st.number_input("Pickup latitude",min_value=-90.0,max_value=90.0,value=float(city_df['pickup_lat'].median()),format="%.6f")
        pickup_lng=st.number_input("Pickup longitude",min_value=-180.0,max_value=180.0,value=float(city_df['pickup_lng'].median()),format="%.6f")
    with c3:
        st.markdown("**Drop-off location**")
        drop_lat=st.number_input("Drop-off latitude",min_value=-90.0,max_value=90.0,value=float(city_df['drop_lat'].median()),format="%.6f")
        drop_lng=st.number_input("Drop-off longitude",min_value=-180.0,max_value=180.0,value=float(city_df['drop_lng'].median()),format="%.6f")
    ...
    st.caption(f"Straight-line coordinate distance: {calculated:.2f} km (for reference only).")
    distance=st.slider("Planned trip distance (km)",0.1,float(max(30,df['distance_km'].max())),7.0,0.1)
    st.caption("Enter the planned route distance. The supplied training distances do not match the coordinates, so straight-line distance is not substituted automatically.")
